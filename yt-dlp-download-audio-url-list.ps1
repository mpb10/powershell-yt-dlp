...
param(
    ...
    [string]
    $YtDlpOptions = "--output '$([environment]::GetFolderPath('MyVideos'))/yt-dlp/%(uploader)s/%(uploader)s - %(title)s.%(ext)s' --download-archive '$([environment]::GetFolderPath('UserProfile'))\scripts\powershell-yt-dlp\var\download-archive.txt' --no-mtime --extract-audio --audio-format mp3 --audio-quality 0"
)
