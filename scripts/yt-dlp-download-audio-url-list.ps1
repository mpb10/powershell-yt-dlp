...
.EXAMPLE 
	yt-dlp-download-audio-url-list.ps1
	    Runs the script using default parameter values.

    yt-dlp-download-audio-url-list.ps1 -Path 'C:\Users\John\scripts\powershell-yt-dlp\etc\audio-url-list.txt' -YtDlpOptions "--output 'C:/Users/John/Music/yt-dlp/%(uploader)s/%(title)s.%(ext)s' --download-archive 'C:\Users\John\scripts\powershell-yt-dlp\var\download-archive.txt' --no-mtime --embed-metadata --extract-audio --audio-format mp3 --audio-quality 0"
...
param(
    ...
    [string]
    $YtDlpOptions = "--output '$([environment]::GetFolderPath('MyMusic'))/yt-dlp/%(uploader)s/%(title)s.%(ext)s' --download-archive '$([environment]::GetFolderPath('UserProfile'))\scripts\powershell-yt-dlp\var\download-archive-audio.txt' --no-mtime --embed-metadata --extract-audio --audio-format mp3 --audio-quality 0"
)
